...
fn test_jit_add() {
    ...
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x3);
    }
}
...
fn test_jit_alu64_arith() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x2a);
    }
}
...
fn test_jit_alu64_bit() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x11);
    }
}
...
fn test_jit_arsh32_high_shift() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x4);
    }
}
...
fn test_jit_arsh() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0xffff8000);
    }
}
...
fn test_jit_arsh64() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0xfffffffffffffff8);
    }
}
...
fn test_jit_call() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x0102030405);
    }
}
...
fn test_jit_call_save() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x4321);
    }
}
...
fn test_jit_div64_imm() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x300000000);
    }
}
...
fn test_jit_err_call_unreg() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        vm.execute_program_jit(&mut []).unwrap();
    }
}
...
fn test_jit_err_div64_by_zero_reg() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0xffffffffffffffff);
    }
}
...
fn test_jit_exit() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x0);
    }
}
...
fn test_jit_ja() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x1);
    }
}
...
fn test_jit_lddw() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x1122334455667788);
    }
}
...
fn test_jit_lddw2() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x80000000);
    }
}
...
fn test_jit_lsh_reg() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x10);
    }
}
...
fn test_jit_mod() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x5);
    }
}
...
fn test_jit_mod64() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x30ba5a04);
    }
}
...
fn test_jit_mul32_imm() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0xc);
    }
}
...
fn test_jit_mul64_imm() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x100000004);
    }
}
...
fn test_jit_mul_loop() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x75db9c97);
    }
}
...
fn test_jit_neg64() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0xfffffffffffffffe);
    }
}
...
fn test_jit_neg() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0xfffffffe);
    }
}
...
fn test_jit_rhs32() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x00ffffff);
    }
}
...
fn test_jit_stack() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0xcd);
    }
}
...
fn test_jit_stack2() {
    ...
    vm.jit_compile().unwrap();
    unsafe {
        assert_eq!(vm.execute_program_jit(&mut []).unwrap(), 0x01020304);
    }
}
